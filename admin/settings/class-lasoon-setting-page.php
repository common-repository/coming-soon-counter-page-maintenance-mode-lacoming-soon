...
if ( ! class_exists( 'Lasoon_Settings_Page', false ) ) :

	/**
	 * Lasoon_Settings_Page.
	 */
	abstract class Lasoon_Settings_Page {
		...
		public function __construct() {
			add_filter( 'lasoon_settings_tabs_array', array( $this, 'add_settings_page' ), 20 );
			add_action( 'lasoon_sections_' . $this->id, array( $this, 'output_sections' ) );
			add_action( 'lasoon_settings_' . $this->id, array( $this, 'output' ) );
			add_action( 'lasoon_settings_save_' . $this->id, array( $this, 'save' ) );
		}
		 ...
		 *
		 * External settings classes (registered via 'lasoon_get_settings_pages' filter)
		...
		final public function get_settings_for_section( $section_id ) {
			...
			return apply_filters( 'lasoon_get_settings_' . $this->id, $settings, $section_id );
		}
		 ...
		 *
		 * When overriding, note that the 'lasoon_get_settings_' filter must NOT be triggered,
		...
		public function get_sections() {
			...
			return apply_filters( 'lasoon_get_sections_' . $this->id, $sections );
		}
		...
		public function output_sections() {
			...
			foreach ( $sections as $id => $label ) {
				$url       = admin_url( 'admin.php?page=main-lasoon&tab=' . esc_attr($this->id, 'lasoon') . '&section=' . sanitize_title( $id ) );
				$class     = ( $current_section === $id ? 'current' : '' );
				$separator = ( end( $array_keys ) === $id ? '' : '|' );
				$text      = esc_html__( $label , 'lasoon');
				echo "<li><a href='".esc_url($url)."' class='".esc_attr($class)."'>".esc_html__($text, 'lasoon')."</a> ".esc_html__($separator, 'lasoon')." </li>"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		public function output() {
			...
			Lasoon_Admin_Settings::output_fields( $settings );
		}

		/**
		 * Save settings and trigger the 'lasoon_update_options_'.id action.
		 */
		...
		protected function save_settings_for_current_section() {
			...
			// for compatibility with derived classes overriding "get_settings".
			$settings = $this->get_settings( $current_section );
			Lasoon_Admin_Settings::save_fields( $settings );
		}

		/**
		 * Trigger the 'lasoon_update_options_'.id action.
		 *
		...
		protected function do_update_options_action( $section_id = null ) {
			...
			if ( $section_id ) {
				do_action( 'lasoon_update_options_' . $this->id . '_' . $section_id );
			}
		}
	}
